...
# EC2 Sandbox - 环境准备脚本
# 用于快速创建基于 AWS EC2 沙盒环境并生成配置文件

set -e

# 全局变量
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"      # 脚本目录
DEFAULT_INSTANCE_TYPE="t3a.small"             # 默认实例类型
DEFAULT_OS_NAME="Ubuntu 24.04 LTS"        # 默认操作系统
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo "  $0 -t c7i.small                      # 使用c7i.small实例"
                echo "  $0 -r us-west-2 -p my-profile        # 指定区域和配置文件"
                echo "  $0 -t m7a.large --skip-test          # 使用m7a.large并跳过测试"
                exit 0
                ;;
            *)
                log_error "未知参数: $1"
